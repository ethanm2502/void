...
$userid = (int)$_GET['assetId'];
$filequery = $con->prepare("SELECT * FROM `library` WHERE `id` = :id");
$filequery->execute(['id' => $userid]);
$files = $filequery->fetch();
$type = $files['type2'];
$fileid = $files['fileid'];
...
$yes = json_decode(threedeecatthumbnail($fileid,$type),true);
